...
- **AI / Pipeline:** LangGraph, Mistral AI, Serper (Web Search)
...
- A valid Mistral API Key (`MISTRAL_API_KEY`).
- A valid Serper API Key (`SERPER_API_KEY`) for web search queries.
...
3. **Synthesis & Search:** LangGraph uses Serper to search the web for missing manufacturer data, then passes the context to Mistral AI. Mistral normalizes brand names, synthesizes missing data, and categorizes the product into strict Pydantic JSON schemas.
